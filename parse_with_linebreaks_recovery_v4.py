...
def _sort_text_refs_by_position(doc_dict: dict, text_refs: List[str]) -> List[Tuple[str, float, float, int]]:
    """
    Sort text refs by visual reading order: page asc, y desc, x asc.
    Returns list of tuples: (ref, y_sort, x_sort, page_no)
    """
    ...
    for ref in text_refs:
        ...
        if not elem:
            continue
        provs = elem.get("prov", []) or []
        if not provs:
            out.append((ref, 0.0, 0.0, 0))
            continue
        prov = provs[0]
        bbox = (prov.get("bbox") or {})
        page_no = int(prov.get("page_no") or 0)
        # NOTE: docling bbox has origin bottom-left; higher 't' means higher on the page.
        t = float(bbox.get("t") or 0.0)
        l = float(bbox.get("l") or 0.0)
        out.append((ref, -t, l, page_no))
    out.sort(key=lambda x: (x[3], x[1], x[2]))
    return out


def _bbox_wh(bbox: dict) -> Tuple[float, float]:
    """Return width/height for a docling bbox, regardless of coord origin."""
    if not bbox:
        return 0.0, 0.0
    try:
        w = float(abs((bbox.get("r") or 0.0) - (bbox.get("l") or 0.0)))
        h = float(abs((bbox.get("t") or 0.0) - (bbox.get("b") or 0.0)))
        return w, h
    except Exception:
        return 0.0, 0.0


def _looks_like_tiny_icon_ocr(text: str, bbox: dict, pic_bbox: Optional[dict]) -> bool:
    """
    Filter out common OCR artifacts coming from UI icons (chat bubbles, etc.).

    We only use *very conservative* heuristics:
    - 1-2 chars total
    - limited to 'o' / '0'
    - small near-square bbox
    - occupies a tiny fraction of the picture area (if known)
    """
    s = (text or "").strip()
    if not s:
        return True
    ...
    if pic_bbox:
        pw, ph = _bbox_wh(pic_bbox)
        parea = pw * ph
        if parea > 0 and (area / parea) > 0.01:
            return False

    return True


def extract_picture_text_lines(
    doc_dict: dict,
    picture_ref: str,
    filter_ocr_artifacts: bool = True,
) -> List[str]:
    """
    Extract textual content that Docling associated with a picture (e.g. OCR output)
    and return it as a list of lines, sorted by reading order.

    This does NOT run OCR itself; it only uses Docling's extracted children texts.
    """
    pic = get_element_by_ref(doc_dict, picture_ref)
    if not pic:
        return []

    child_text_refs: List[str] = []
    for ch in (pic.get("children") or []):
        ref = (ch or {}).get("$ref", "")
        if ref.startswith("#/texts/"):
            child_text_refs.append(ref)

    if not child_text_refs:
        return []

    # picture bbox (used for conservative artifact filtering)
    pic_prov = (pic.get("prov") or [{}])[0] if isinstance(pic.get("prov"), list) else {}
    pic_bbox = (pic_prov.get("bbox") or {}) if isinstance(pic_prov, dict) else {}

    sorted_refs = _sort_text_refs_by_position(doc_dict, child_text_refs)

    lines: List[str] = []
    prev_was_header = False
    for ref, _, _, _page in sorted_refs:
        t = get_element_by_ref(doc_dict, ref)
        if not t:
            continue
        ...
        if filter_ocr_artifacts:
            prov = (t.get("prov") or [{}])[0] if isinstance(t.get("prov"), list) else {}
            bbox = (prov.get("bbox") or {}) if isinstance(prov, dict) else {}
            if _looks_like_tiny_icon_ocr(text, bbox, pic_bbox):
                continue

        label = (t.get("label") or "").strip()
        is_header = label == "section_header"
        # add a blank line before image-internal headers for readability
...
def extract_picture_alt_text_lines(doc_dict: dict, picture_ref: str) -> List[str]:
    """
    Extract caption/annotation text for a picture if present.
    (Some PDFs include accessibility alt text or figure captions.)
    """
    pic = get_element_by_ref(doc_dict, picture_ref)
    if not pic:
        return []
    ...
    def _collect_text_from_maybe_refs(items: list):
        for it in items or []:
            if isinstance(it, dict) and "$ref" in it:
                elem = get_element_by_ref(doc_dict, it["$ref"])
                if elem and elem.get("text"):
                    out.append(str(elem.get("text")).strip())
            elif isinstance(it, dict) and it.get("text"):
                out.append(str(it.get("text")).strip())
            elif isinstance(it, str):
                out.append(it.strip())

    _collect_text_from_maybe_refs(pic.get("captions") or [])
    _collect_text_from_maybe_refs(pic.get("annotations") or [])

    out = [t for t in out if t]
    return out


def _already_contains_any(lines: List[str], needle_lines: List[str]) -> bool:
    """Heuristic: if any non-trivial extracted line already exists in nearby markdown, skip injection."""
    hay = "\n".join(lines)
    non_trivial = [t for t in needle_lines if len(t.strip()) > 6]
    for t in non_trivial[:5]:
        if t in hay:
            return True
    return False
...
def remove_repeated_from_markdown(
    ...
    drop_image_marker_when_text: bool = False,
    drop_image_marker_min_chars: int = 80,
    filter_ocr_artifacts: bool = True,
) -> str:
    """
    ...
    while i < len(in_lines):
        ...
        if line.strip() == image_marker:
            ...
            # Determine if we have picture text to inject.
            extracted: List[str] = []
            if include_image_text and pic_ref and pic_ref not in first_occurrence_refs:
                extracted = extract_picture_text_lines(
                    doc_dict,
                    pic_ref,
                    filter_ocr_artifacts=filter_ocr_artifacts,
                )

            extracted_chars = sum(len(x.strip()) for x in extracted if x and x.strip())
            should_drop_marker = bool(
                drop_image_marker_when_text
                and extracted
                and extracted_chars >= int(drop_image_marker_min_chars)
            )

            # Keep the marker unless explicitly configured to drop it.
            if not should_drop_marker:
                new_lines.append(line)

            # Inject picture-associated text (Docling OCR) or a placeholder.
            # We skip pictures that are part of the (first-occurrence) header to avoid duplicating
            # the header block we insert later.
            if include_image_text and pic_ref and pic_ref not in first_occurrence_refs:
                lookahead = in_lines[i+1:i+40]
                if extracted and not _already_contains_any(lookahead, extracted):
                    # Ensure a visual break from surrounding text.
                    if new_lines and new_lines[-1] != "":
                        new_lines.append("")
                    new_lines.extend(extracted)
                    new_lines.append("")
                elif include_image_placeholders and not extracted:
                    alt_lines = extract_picture_alt_text_lines(doc_dict, pic_ref)
                    if new_lines and new_lines[-1] != "":
                        new_lines.append("")
                    if alt_lines:
                        new_lines.extend(alt_lines)
                    else:
                        new_lines.append("[Image: no extractable text]")
                    new_lines.append("")
...
def main():
    ...
    # Image text injection options
    ap.add_argument("--no-image-text", action="store_true",
                    help="Do not inject Docling-extracted text that is associated with images.")
    ap.add_argument("--image-placeholders", action="store_true",
                    help="If an image has no extractable text/metadata, insert a placeholder line after the image marker.")

    ap.add_argument("--drop-image-marker-when-text", action="store_true",
                    help="If an image has substantial extracted text, omit the '<!-- image -->' marker for that image.")
    ap.add_argument("--drop-image-marker-min-chars", type=int, default=80,
                    help="Minimum number of extracted characters required before dropping the image marker (default: 80).")
    ap.add_argument("--no-ocr-artifact-filter", action="store_true",
                    help="Disable filtering of tiny OCR artifacts (e.g., UI icons) inside images.")
    ...
    if dedupe_enabled:
        ...
        if repeated_refs or first_occurrence_refs:
            ...
            deduped_md = remove_repeated_from_markdown(
                ...
                doc_dict,
                repeated_refs,
                first_occurrence_refs,
                include_image_text=not args.no_image_text,
                include_image_placeholders=args.image_placeholders,
                drop_image_marker_when_text=args.drop_image_marker_when_text,
                drop_image_marker_min_chars=args.drop_image_marker_min_chars,
                filter_ocr_artifacts=not args.no_ocr_artifact_filter,
            )
